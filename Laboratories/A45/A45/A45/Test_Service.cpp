...
void Test_Service::TestAddUserService() {
    ...
    assert((*UserRepo->GetDArray())[0].GetSize() == "XL");
    assert((*UserRepo->GetDArray())[0].GetColour() == "Pink");
    assert((*UserRepo->GetDArray())[0].GetPrice() == 100);
    assert((*UserRepo->GetDArray())[0].GetQuantity() == 1);
    assert((*UserRepo->GetDArray())[0].GetPhoto() == "https://abcd.com");
    assert(service->AddUserService(c, cUser) == 0);
    assert((*UserRepo->GetDArray())[0].GetQuantity() == 2);
    Coat cc("XL", "Purple", 100, 0, "https://sag.com");
    assert(service->AddUserService(cc, cUser) == 0);
    delete service;
}
