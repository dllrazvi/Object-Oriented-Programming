	...
	:service{ s }, person{ p }, QMainWindow(parent)
{
	...
	this->nearbyCheckBox = new QCheckBox("Show Nearby Events", this);
	this->ui.centralWidget->layout()->addWidget(this->nearbyCheckBox);

	// Connect the checkbox signal to the appropriate slot
	QObject::connect(this->nearbyCheckBox, &QCheckBox::stateChanged, this, &Exam::nearbyCheckBoxStateChanged);
}
void Exam::nearbyCheckBoxStateChanged()
{
    this->populateList();
}
